...
素材来自该会话作用域内的长期记忆、周记与对话缓冲：前者是用户侧内容，对话缓冲则同时含
用户发言（``用户(昵称)：``）与 Bot 自己的发言（``我：``），因此渲染时标注了说话者。
素材为空时**宁可不发**（而不是硬编一句问候）。
"""
...
class ProactiveMaterial:
    ...
    def render(self, *, max_chars: int = 1200) -> str:
        ...
        for title, items in (
            ("长期记忆", self.memories),
            ("现实周记", self.journals),
            # 对话缓冲两侧发言都在里面，标注说话者，免得模型把自己的话当成用户说的。
            ("最近对话（「我」是你自己说的，「用户(昵称)」是用户说的）", self.buffers),
        ):
            if not items:
                continue
