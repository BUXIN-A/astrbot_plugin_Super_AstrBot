...
| 提示词定制 | 把反思、周度洞察、上下文摘要、主动消息、黑话、好感度、图谱、自动审核的**系统提示词与模板**都开放为配置项，留空即用内置默认（见下文「提示词定制」） |
...
在 AstrBot 插件详情页 → **Pages** → `dashboard` 打开，共十个分区：
...
- **提示词定制**：反思/周度/摘要/主动消息/黑话/好感度/图谱/自动审核的系统提示词与模板（留空用内置默认）
...
所有提示词都可以在插件配置页 → **提示词定制** 里覆盖，**留空即用内置默认**。

- 每个字段是多行文本框，`hint` 里写明该模板**必须保留的占位符**；
- 模板缺少占位符、或花括号写错 → 该模板被忽略并**回退内置默认**，启动日志会告警一次，
  **不会**因此报错或影响对话；
...
- [ ] 在服务器上验证运行监控：面板「监控」分区指标随对话增长，切换时间范围/粒度正常，
      重启后历史趋势仍在（已落盘）
- [ ] 在服务器上验证提示词定制：填入合法模板后日志/效果随之改变；故意删掉占位符后
      启动日志出现「已回退内置默认」告警且对话正常
